...
- If `graphify-out/graph.json` is **missing** (check under the profile root, not `$HOME`):
  1. Prefer the **camp baseline** (fast): run `scripts/setup-graphify.sh` — if `graphify-baseline.tar.gz` is present it unpacks immediately (no multi-hour extract).
  2. Otherwise tell the user setup takes a **long time** (often 30+ minutes on Thor; can be hours) — do **not** run extract in a blocking/foreground tool that will timeout.
  3. Resolve the setup script with an **absolute path**, then run it **once** (background by default when no baseline):
...
bash "$SETUP"           # baseline unpack OR backgrounds extract
bash "$SETUP" --status
```

  4. Ready when `"$(dirname "$SETUP")/../graphify-out/graph.json"` exists (i.e. profile `graphify-out/graph.json`). Follow with `tail -f "$(dirname "$SETUP")/../graphify-out/setup.log"` only if a long extract is running.
  5. Do **not** re-run setup in a loop waiting in the foreground; continue other work or poll `--status`.
  6. Do **not** use a relative `bash scripts/setup-graphify.sh` unless you have already `cd`'d into the profile root (`…/hermes-profile` or `~/.hermes/profiles/sage`).
  7. Full rebuild only when needed: `bash "$SETUP" --rebuild` (ignore baseline; long LLM extract).- Dirty `graphify-out/` files are expected after hooks or incremental updates; dirty graph files are not a reason to skip graphify. Only skip if the task is about stale/incorrect graph output, or the user explicitly says not to use it.
