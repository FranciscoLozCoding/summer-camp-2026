...
- If `graphify-out/graph.json` is **missing**, tell the user and run (or guide) `scripts/setup-graphify.sh` / `graphify extract . --backend ollama` from the profile root before inventing skill names.
